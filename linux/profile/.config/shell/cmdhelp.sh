...
blkid # Show block device UUIDs/types
cat /etc/os-release # Distribution info
cat file # Print file contents
cd - # Switch to previous directory
chmod +x script.sh # Make file executable
chmod 640 file # Set permissions (rw-r-----)
chown user:group file # Change owner/group
cp -av src dest # Copy (archive mode, verbose)
...
date -Iseconds # ISO-8601 timestamp
...
du -sh * # Disk usage per item (current dir)
...
findmnt # Show mounts in a tree (more readable than mount)
free -h # Memory usage
git add -A # Stage all changes
git commit -m "message" # Commit staged changes
git diff # Show uncommitted changes
git log --oneline --decorate -n 20 # Recent commits (compact)
git status # Show git working tree status
grep -RIn "PATTERN" . # Search text recursively (grep)
groups # Group memberships
gunzip file.gz # Decompress gzip (alias of gzip -d)
gzip -9 file # Maximum compression (slower)
gzip -c file > file.gz # Write gzip output to stdout
...
id # User/group IDs
ifconfig -a # Show interfaces (legacy net-tools; prefer ip a)
...
ls -lah # List files (human, all, long)
...
nc -l -p <port> # Start a TCP listener on a port (traditional netcat syntax)
nc -lv 0.0.0.0 <port> # Start a TCP listener on all interfaces (OpenBSD netcat syntax)
nc -ulv 0.0.0.0 <port> # Start a UDP listener on all interfaces (OpenBSD netcat syntax)
nc -vz host 443 # Test TCP port reachability (netcat)
netstat -natup | grep LISTEN | sort -k 7 # List listening ports and sort by program name (legacy net-tools)
...
sort file | uniq -c | sort -nr # Count unique lines (top first)
source ~/.bashrc # Reload bashrc
ss -tulpn # Listening sockets and PIDs
ssh -D 127.0.0.1:1080 -N user@bastion # SOCKS proxy (dynamic tunnel; configure browser to use 127.0.0.1:1080)
ssh -i ~/.ssh/id_ed25519 user@host # SSH with specific key
ssh -J user@bastion user@target # SSH to target via bastion (Jump host)
ssh -L 127.0.0.1:5432:db.internal:5432 user@bastion # Local tunnel (local 5432 -> db.internal:5432 via bastion)
ssh -L 127.0.0.1:8080:127.0.0.1:80 user@host # Local tunnel to a remote localhost service
ssh -N -f -L 127.0.0.1:3306:127.0.0.1:3306 user@host # Background local tunnel (-N no shell, -f background)
ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=3 user@host # Keepalive to reduce idle disconnects
ssh user@host # SSH to remote host
ssh-add ~/.ssh/id_ed25519 # Add key to ssh-agent
ssh-keygen -l -f ~/.ssh/id_ed25519.pub # Show public key fingerprint
ssh-keygen -t ed25519 -a 100 -f ~/.ssh/id_ed25519 -C "you@example.com" # Generate ed25519 key (use a passphrase; stronger KDF rounds)
...
sudo apt remove -y <pkg> # Remove package (Debian/Ubuntu)
...
sudo yum update -y # Update packages (legacy RHEL/CentOS)
sudoedit /etc/hosts # Edit a root-owned file safely using your editor
tail -f file # Follow a file (tail -f)
tail -n 200 -f /var/log/syslog # Follow syslog (Debian/Ubuntu)
tail -n 200 file # Show last 200 lines
